...
import com.itheima.backend.api.aliyunai.AliYunAiApi;
import com.itheima.backend.api.aliyunai.model.CreateOutPaintingTaskRequest;
...
import com.itheima.backend.manager.FileManager;
import com.itheima.backend.manager.upload.FilePictureUpload;
import com.itheima.backend.manager.upload.PictureUploadTemplate;
import com.itheima.backend.manager.upload.UrlPictureUpload;
import com.itheima.backend.mapper.PictureMapper;
...
import com.itheima.backend.model.enums.PictureReviewStatusEnum;
...
import com.itheima.backend.utils.ColorSimilarUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null) {
            ...
            if (spaceId == null) {
                ...
                if (ObjUtil.notEqual(spaceId, oldPicture.getSpaceId())) {
                    throw new BusinessException(ErrorCode.PARAM_ERROR, "空间 id 不一致");
                }
            }
        }
        ...
        if (inputSource instanceof MultipartFile) {
            ...
            throw new BusinessException(ErrorCode.PARAM_ERROR, "不支持的上传类型");
        }
        if(inputSource == null){
            throw new BusinessException(ErrorCode.PARAM_ERROR, "图片为空");
        }
...
// 开启事务
    ...
    public void validPicture(Picture picture) {
        ...
        ThrowUtils.throwIf(ObjUtil.isNull(id), ErrorCode.PARAM_ERROR, "id 不能为空");
        if (StrUtil.isNotBlank(url)) {
            ThrowUtils.throwIf(url.length() > 1024, ErrorCode.PARAM_ERROR, "url 过长");
        }
        if (StrUtil.isNotBlank(introduction)) {
            ThrowUtils.throwIf(introduction.length() > 800, ErrorCode.PARAM_ERROR, "简介过长");
        }
    }
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAM_ERROR, "最多 30 条");
        // 要抓取的地址
...
// 开启事务
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        // 1. 校验参数
        ThrowUtils.throwIf(spaceId == null || CollUtil.isEmpty(pictureIdList), ErrorCode.PARAM_ERROR);
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NO_AUTH_ERROR);
        // 2. 校验空间权限
        Space space = spaceService.getById(spaceId);
        ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
